    ...
    try {
        ...
        ProductTypeModel
            ...
            .exec((err, doc) => {
                ...
                ProductTypeModel.countDocuments(query).exec((count_error, count) => {
                    if (err) {
    ...
    try {
        ...
        form.parse(req,  async function (err, fields, files) {
            try {
                ...
                const itemSaved = await ProductTypeModel({
                    name: name,
                    code: code,
                }).save();
...
productTypeController.edit = async (req, res, next) => {
    try {
        ...
        let item = await ProductTypeModel.findById(paramId);
        if (item == null) {
        ...
        form.parse(req,  async function (err, fields, files) {
            try {
                ...
                const itemSaved = await ProductTypeModel.findByIdAndUpdate(paramId, {
                    name: name,
                    code: code,
                });
                return res.status(httpStatus.OK).json({
...
productTypeController.delete = async (req, res, next) => {
    try {
        ...
        let paramId = req.params.id;
        let item = await ProductTypeModel.findByIdAndDelete(paramId);
        if (item == null) {
            return res.status(httpStatus.NOT_FOUND).json({
                message: req.language === LANGUAGE_EN ? 'Not found!' : 'Không tìm thấy!'
            });
