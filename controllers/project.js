    ...
    try {
        ...
        ProjectModel
            ...
            .exec((err, doc) => {
                ...
                ProjectModel.countDocuments(query).exec((count_error, count) => {
                    if (err) {
    ...
    try {
        ...
        form.parse(req,  async function (err, fields, files) {
            try {
                ...
                let dataEquipments = Array.isArray(JSON.parse(equipments)) ? JSON.parse(equipments) : []
                ...
                const typeFind = type ? await ProjectTypeModel.findById(type) : null;
                const clientFind = client ? await ClientModel.findById(client) : null;
                const itemSaved = await ProjectModel({
                    name: name,
                    code: code,
                    ...
                    equipments: dataEquipments.map((item, index) => {
                        return ({
...
projectController.edit = async (req, res, next) => {
    try {
        ...
        let item = await ProjectModel.findById(paramId);
        if (item == null) {
        ...
        form.parse(req,  async function (err, fields, files) {
            try {
                ...
                const typeFind = type ? await ProjectTypeModel.findById(type) : null;
                const clientFind = client ? await ClientModel.findById(client) : null;
                const itemSaved = await ProjectModel.findByIdAndUpdate(paramId, {
                    name: name,
                    code: code,
                    description: description,
                    startDate: startDate,
...
projectController.delete = async (req, res, next) => {
    try {
        ...
        let paramId = req.params.id;
        let item = await ProjectModel.findByIdAndDelete(paramId);
        if (item == null) {
            return res.status(httpStatus.NOT_FOUND).json({
                message: req.language === LANGUAGE_EN ? 'Not found!' : 'Không tìm thấy!'
            });
