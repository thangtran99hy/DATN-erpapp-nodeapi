    ...
    try {
        ...
        EquipmentTypeModel
            ...
            .exec((err, doc) => {
                ...
                EquipmentTypeModel.countDocuments(query).exec((count_error, count) => {
                    if (err) {
    ...
    try {
        ...
        form.parse(req,  async function (err, fields, files) {
            try {
                ...
                const itemSaved = await EquipmentTypeModel({
                    name: name,
                    code: code,
                }).save();
...
equipmentTypeController.edit = async (req, res, next) => {
    try {
        ...
        let item = await EquipmentTypeModel.findById(paramId);
        if (item == null) {
        ...
        form.parse(req,  async function (err, fields, files) {
            try {
                ...
                const itemSaved = await EquipmentTypeModel.findByIdAndUpdate(paramId, {
                    name: name,
                    code: code,
                });
                return res.status(httpStatus.OK).json({
...
equipmentTypeController.delete = async (req, res, next) => {
    try {
        ...
        let paramId = req.params.id;
        let item = await EquipmentTypeModel.findByIdAndDelete(paramId);
        if (item == null) {
            return res.status(httpStatus.NOT_FOUND).json({
                message: req.language === LANGUAGE_EN ? 'Not found!' : 'Không tìm thấy!'
            });
